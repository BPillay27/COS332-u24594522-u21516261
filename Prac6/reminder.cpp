...
class serviceSMNTP{
    private:
        int parseCode(const char *buf) { //Function to check if therer is a success code else throws an exception
        ...
        std::string recvLineCRLF(int sock) { //Function to receive response from SMNTP SERVER and checks the size to see if valid response for error handling
            std::string s;
            char c;
            while (true) {
                ssize_t r = recv(sock, &c, 1, 0);
                if (r <= 0) throw std::runtime_error("recv error/closed");
                s.push_back(c);
        ...
        int recvSMTPResponse(int sock, std::string &out) {//Will check if the response is correct for that command
            out = recvLineCRLF(sock);
            if (out.size() < 4) throw std::runtime_error("short SMTP response");
    ...
    public:
        void sendEmail(const string& user, const vector<string>& events) {
            ...
            if (code != 220) throw std::runtime_error("Expected 220 greeting: " + response);

            string msg="HELO localhost\r\n";// The SMTP Handshake
            send(sock, msg.c_str(), msg.length(), 0);
            code = recvSMTPResponse(sock, response);
            if (code != 250) throw std::runtime_error("Expected 250 for OK: " + response);
            ...
            send(sock, msg.c_str(), msg.length(), 0);
            code = recvSMTPResponse(sock, response);
            if (code != 354) throw std::runtime_error("Expected 354 for starting mail input: " + response);

            // Construct the email body
            string content = "Subject: Event Reminder\r\n\r\n";
            content += "Events happening in 6 days:\n";
            for(const auto& e : events) content += "- " + e + "\n";
            ...
            send(sock, msg.c_str(), msg.length(), 0);
            code = recvSMTPResponse(sock, response);
            if (code != 250) throw std::runtime_error("Expected 250 for OK after content: " + response);

            
            msg="QUIT\r\n";
            send(sock, msg.c_str(), msg.length(), 0);
            code = recvSMTPResponse(sock, response);
            if (code != 221) throw std::runtime_error("Expected 221 to know that the connection is closed: " + response);
...
static std::string date_plus_days(int days){// Returns date in "dd/mm" for `days` days from now
    std::time_t t = std::time(nullptr);
    std::tm tm{};
    localtime_r(&t, &tm);
    tm.tm_mday += days;
...
static std::string readUser(){
    ...
    while (std::getline(f, line)) {
        ...
        if (val_first == std::string::npos) val = "";
        else {
        ...
        if (key == "maiUser" || key == "mailUser") {
            if (val.empty()) throw std::runtime_error("maiUser value empty in .env");
            return val;
        }
    }
